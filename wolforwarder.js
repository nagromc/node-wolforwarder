...
function magicPacketToString(msg) {
    ...
    for (i = MGC_PKT_START_OFFSET; i < MGC_PKT_REPEAT_MAC_ADDR_NUMBER + MGC_PKT_START_OFFSET; i += 1) {
...
function controlMagicPacket(msg) {
    var sync, prevSeq, seq, i;

    sync = macaddr.toString(msg.slice(0, macaddr.LENGTH));
